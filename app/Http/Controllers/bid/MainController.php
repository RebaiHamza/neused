...
class MainController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($sellerSystem == 1) {
            $products = $productsquery->orderBy('id', 'DESC')->where('status', '1')->where('is_bid','1')
                ->take(20)
                ->get();
        } else {
       ...
       // dd($products);
       //  dd($advs   );
        return view('front.index-bid', compact('slider', 'advs', 'products', 'featureds', 'home_slider', 'blogs', 'conversion_rate'));
