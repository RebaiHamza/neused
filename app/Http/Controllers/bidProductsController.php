...
class bidProductsController extends Controller
{
        ...
        if ($request->ajax()) {
            return DataTables::of($products)
                ...
                ->addColumn('image', function ($row) {
                    ...
                    if ($row->mainimage) {
                        $image .= '<img title='.str_replace('"', '', $row->productname).' class="pro-img" src='.url('bidProducts/thumbnails/'.$row->mainimage).' alt='.$row->mainimage.'>';
                    } else {
    ...
    {
        $path = public_path().'/bidProducts';

        $thumbpath = public_path().'/bidProducts/thumbnails';
        ...
        if ($file = $request->file($image)) {
            ...
            $name = 'bidProd_'.time().str_random(10).'.'.$file->getClientOriginalExtension();
            $img = Image::make($file);
    ...
    //    ]);
        ...
        return redirect()->route('bid-products.index')->with('success', 'Product created ! ');
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('admin.bidProduct.edit_tab', compact('rel_setting', 'products', 'categorys', 'owners', 'brands', 'faqs', 'child', 'grand'));
    }
