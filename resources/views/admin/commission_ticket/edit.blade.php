        ...
        <div class="box-header with-border">
          <h3 class="box-title"> Edit Commision</h2>
                    <div class="panel-heading">
                          <a href=" {{url('admin/commissionticket/')}} " class="btn btn-success pull-right owtbtn">< Back</a> 
                        </div>   
                    <div class="clearfix"></div>
                  </div>
                  <div class="x_content">
                    <br />
                    <form id="demo-form2" method="post" enctype="multipart/form-data" action="{{url('admin/commissionticket/'.$commission->id.'/store_edit')}}" data-parsley-validate class="form-horizontal form-label-left">
                        {{csrf_field()}}
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">
                          ticket Product <span class="required">*</span>
                        </label>
                       <div class="col-md-6 col-sm-6 col-xs-12">
                          <select name="ticket_id" class="form-control col-md-7 col-xs-12" id="country_id">
                          <option value="0">Please Choose</option>
                            @foreach($product as $pro)
                              <option value="{{$pro->id}}" {{ $commission->ticket_id == $pro->id ? 'selected="selected"' : '' }}>
                                {{$pro->name}}
                              </option>
                            @endforeach
                          </select>
                           <small class="txt-desc">(Please Choose ticket Product)</small>
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">
