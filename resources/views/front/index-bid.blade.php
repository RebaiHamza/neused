          ...
          <div class="tab-pane" id="pro{{ $parentcats->id }}">
            ...
            <div class="product-slider">
              ...
              <div class="owl-carousel home-owl-carousel custom-carousel owl-theme">
                ...
                @foreach(App\Product::where('category_id',$parentcats->id)->where('is_bid','1')->take(10)->get() as $pro)
                @if($genrals_settings->vendor_enable != 1)
                @if($pro->vender['role_id'] == 'a')
                @foreach($pro->subvariants as $key=> $orivar)
                @if($orivar->def ==1)
                ...
                for($i = 0; $i<$var_name_count; $i++){ $var_id=$orivar['main_attr_id'][$i];
                  ...
                  try{
                  $url =
                  url(url('details-bid').'/').$pro->id.'?'.$name[0]['attr_name'].'='.$var_name[0].'&'.$name[1]['attr_name'].'='.$var_name[1];
                  }catch(Exception $e)
                  {
                  $url = url(url('details-bid').'/').$pro->id.'?'.$name[0]['attr_name'].'='.$var_name[0];
                  }
